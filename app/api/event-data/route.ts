  ...
  try {
    ...
    for (const [assetName, assetTicker] of Object.entries(ASSET_TICKERS)) {
      try {
        ...
        // If we don't have enough data, try multiple Yahoo Finance approaches
        if (priceData.length === 0 || !hasCompleteDateRange(priceData, startDate, endDate)) {
          ...
          const yahooAttempts = [
            () => fetchYahooDataV1(assetTicker, startDate, endDate),
            () => fetchYahooDataV2(assetTicker, startDate, endDate),
            () => fetchYahooDataV3(assetTicker, startDate, endDate),
          ]

          let yahooSuccess = false
          for (let i = 0; i < yahooAttempts.length && !yahooSuccess; i++) {
            try {
              console.log(`Trying Yahoo Finance approach ${i + 1} for ${assetName}...`)
              const yahooData = await yahooAttempts[i]()
              if (yahooData && yahooData.length > 0) {
                priceData = yahooData
                yahooSuccess = true
                console.log(
                  `Yahoo Finance approach ${i + 1} succeeded for ${assetName}: ${priceData.length} data points`,
                )
              }
            } catch (yahooError) {
              console.log(`Yahoo Finance approach ${i + 1} failed for ${assetName}:`, yahooError)
            }
          }

          if (!yahooSuccess) {
            console.log(`All Yahoo Finance approaches failed for ${assetName}`)
            continue // Skip this asset entirely
          }

          // Only try to save to GitHub if it's available and we have a token
          if (githubAvailable && process.env.GITHUB_TOKEN && priceData.length > 0) {
        ...
        if (priceData.length > 0) {
          ...
          successfulAssets++
          console.log(`Successfully processed ${filteredData.dates.length} data points for ${assetName}`)
        }
      } catch (assetError) {
        console.error(`Error processing ${assetName}:`, assetError)
        // Skip this asset entirely - no fallback
        continue
      }
    }

    console.log(`Returning data for ${successfulAssets} out of ${Object.keys(ASSET_TICKERS).length} assets`)

    if (successfulAssets === 0) {
      return NextResponse.json(
        {
          error:
            "No market data could be retrieved for any assets. This may be due to API access restrictions or network issues.",
          assets: {},
          eventDate,
          dateRange: {
    ...
    return NextResponse.json({
      ...
      warning:
        successfulAssets < Object.keys(ASSET_TICKERS).length
          ? `Only ${successfulAssets} out of ${Object.keys(ASSET_TICKERS).length} assets could be retrieved due to API limitations.`
          : undefined,
    })
  } catch (error) {
...
// Primary Yahoo Finance approach - CSV download
async function fetchYahooDataV1(ticker: string, startDate: Date, endDate: Date) {
  ...
  console.log(`Yahoo Finance V1 URL: ${url}`)
  ...
  console.log(`Yahoo Finance V1 CSV length: ${csvText.length}`)

  return parseCSVData(csvText)
}

// Secondary Yahoo Finance approach - JSON API
async function fetchYahooDataV2(ticker: string, startDate: Date, endDate: Date) {
  const startTimestamp = Math.floor(startDate.getTime() / 1000)
  const endTimestamp = Math.floor(endDate.getTime() / 1000)

  const url = `https://query2.finance.yahoo.com/v8/finance/chart/${ticker}?period1=${startTimestamp}&period2=${endTimestamp}&interval=1d&includePrePost=false&events=div%7Csplit%7Cearn`
  ...
  return parseYahooJSONData(data)
}

// Third Yahoo Finance approach - Alternative endpoint
async function fetchYahooDataV3(ticker: string, startDate: Date, endDate: Date) {
  const startTimestamp = Math.floor(startDate.getTime() / 1000)
  const endTimestamp = Math.floor(endDate.getTime() / 1000)

  const url = `https://finance.yahoo.com/quote/${ticker}/history?period1=${startTimestamp}&period2=${endTimestamp}&interval=1d&filter=history&frequency=1d`

  console.log(`Yahoo Finance V3 URL: ${url}`)
  ...
  if (!response.ok) {
    throw new Error(`Yahoo Finance V3 returned ${response.status}: ${response.statusText}`)
  }

  // This would require HTML parsing - for now, throw an error to move to next approach
  throw new Error("Yahoo Finance V3 HTML parsing not implemented")
}
...
function parseYahooJSONData(data: any) {
  try {
    const result = data.chart?.result?.[0]
    if (!result) {
      throw new Error("No chart data found in JSON response")
    }
    ...
    console.log(`Successfully parsed ${parsedData.length} rows from JSON`)
    return parsedData.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
  } catch (error) {
    console.error("Error parsing Yahoo JSON data:", error)
    return []
  }
}
